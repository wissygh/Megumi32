...
class PrefetchController()(p: IFParams) extends Module with RequireAsyncReset{
    ...
    withReset((!this.reset.asBool).asAsyncReset) {
        ...
        val state = RegCouple(idle.getWidth, Some(idle))
        
        val cnt = RegCouple(p.FIFO_ADDR_DEPTH+1, Some(0.U))     // Transaction counter
        val count_up, count_down = Wire(Bool())                 // In/Decrement outstanding transactions
        
        val flush_cnt = RegCouple(p.FIFO_ADDR_DEPTH+1, Some(0.U))   // Response flush counter (to flush speculative responses after branch)
        ...
        io.fsInf.busy := (cnt.q =/= 0.U(3.W)) || io.transInf.transRespValid
        ...
        /**
          ...
          */
        io.fetch_valid := (fifo_valid || io.transInf.transRespValid) && !(io.fsInf.branch || (flush_cnt.q > 0.U))
        ...
        if (p.PULP_OBI == 0) {
            /**
              ...
              * 
              */
            io.transInf.transRequest.valid := io.fsInf.req && (fifo_cnt_masked + cnt.q < p.DEPTH.U)
        } else {
            /**
              * Legacy PULP OBI behavior, i.e. only issue subsequent transaction if preceding transfer
              * is about to finish (re-introducint timing critial path from instr_rvalid to instr_req)
              */
            io.transInf.transRequest.valid := Mux(cnt.q === 0.U(3.W), 
                                                  io.fsInf.req && (fifo_cnt_masked + cnt.q < p.DEPTH.U),
                                                  io.fsInf.req && (fifo_cnt_masked + cnt.q < p.DEPTH.U) && io.transInf.transRespValid)
        }
        ...
        io.fifoInf.fifo_push := io.transInf.transRespValid && (fifo_valid || !io.fetch_ready) && !(io.fsInf.branch || (flush_cnt.q > 0.U))
        ...
        cnt.n := MuxLookup(Cat(count_up, count_down), cnt.q, 
            Seq(
                0.U(2.W) -> cnt.q,
                1.U(2.W) -> (cnt.q - 1.U(1.W)),
                2.U(2.W) -> (cnt.q + 1.U(1.W)),
                3.U(2.W) -> cnt.q
            )
        )
        ...
        when(io.fsInf.branch) {
            flush_cnt.n := cnt.q
            when(io.transInf.transRespValid && (cnt.q > 0.U)) {
                flush_cnt.n := cnt.q - 1.U(1.W)
            }
        }.elsewhen(io.transInf.transRespValid && (flush_cnt.q > 0.U)) {
            flush_cnt.n := flush_cnt.q - 1.U(1.W)
        }
